...
def evaluate(
    ...
    save_folder=None,
    save_midi=False,
    save_midi_programs=None,
):
    metrics = defaultdict(list)

    csvfile = None
    for label_multi in tqdm(data):
        pred_multi, losses = model.run_on_batch(label_multi)
        ...
        for i in range(pred_multi.onset.shape[-1]):
            ...
            label = MusicAnnotation(
                onset=label_multi.annotation.onset[:, :, i],
                frame=label_multi.annotation.frame[:, :, i],
                offset=label_multi.annotation.offset[:, :, i],
                velocity=label_multi.annotation.velocity[:, :, i],
            )
            for value in pred:
                if value is None:
                    continue
            ...
            p_ref, i_ref, v_ref = extract_notes(label.onset, label.frame, label.velocity, add_onset_modification=True)
            p_est, i_est, v_est = extract_notes(pred.onset, pred.frame, pred.velocity, onset_threshold, frame_threshold)
            if save_midi:
                pitches_dict[save_midi_programs[i]]["pitches"] = p_est
                pitches_dict[save_midi_programs[i]]["intervals"] = i_est
                pitches_dict[save_midi_programs[i]]["velocities"] = v_est
            ...
            if save_folder is not None:
                ...
                track = f"{label_multi.track}-{instruments[i]}"
                ...
                start_time = strftime("%M:%S", gmtime(label_multi.start_time))
                end_time = strftime("%M:%S", gmtime(label_multi.end_time))
                frame_f1 = metrics["metric/frame/f1"][-1]
                note_f1 = metrics["metric/note/f1"][-1]
                note_w_offset_f1 = metrics["metric/note-with-offsets/f1"][-1]
                file_name = (
                    f"{track}-{i}-{start_time}-{end_time}-F1s:{frame_f1:.3f}|{note_f1:.3f}|{note_w_offset_f1:.3f}"
                )
                label_path = os.path.join(save_folder, file_name + ".label.png")
                save_pred_and_label_piano_roll(label_path, model, label_multi.audio, label, pred, pred_notes)

        if save_folder is not None and save_midi:
            start_time = strftime("%M:%S", gmtime(label_multi.start_time))
            end_time = strftime("%M:%S", gmtime(label_multi.end_time))
            file_name = f"{label_multi.track}-{i}-{start_time}-{end_time}"
...
def evaluate_file_on_slakh_amt_dataset(
    ...
    metrics = evaluate(tqdm(dataset), model, instruments, onset_threshold, frame_threshold, save_folder)
...
if __name__ == "__main__":
    ...
    if args.save_folder is None:
        instrument_name = "-".join(args.instruments)
