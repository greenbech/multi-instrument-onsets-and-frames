...
class PianoRollAudioDataset(Dataset):
    def __init__(self, path, groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        ...
        self.data = []
        print(f"Loading {len(groups)} group{'s' if len(groups) > 1 else ''} " f"of {self.__class__.__name__} at {path}")
        for group in groups:
...
class MAESTRO(PianoRollAudioDataset):
    ...
    def files(self, group):
        if group not in self.available_groups():
            ...
            files = [
                (audio if os.path.exists(audio) else audio.replace(".flac", ".wav"), midi) for audio, midi in files
            ]
...
class MAPS(PianoRollAudioDataset):
    def __init__(self, path="data/MAPS", groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(
            path, groups if groups is not None else ["ENSTDkAm", "ENSTDkCl"], sequence_length, seed, device
        )
